...
$app->post('/article/create', function (Request $request) use ($app) {
    ...
    return $app->redirect($app['url_generator']->generate('article_show', ['id' => $article->getId()]));
})->bind('article_create');
...
$app->get('/article/show/{id}', function ($id) use ($app) {
    ...
    return $app['twig']->render('article/show.html.twig', [
        'article' => $article,
    ]);
})->bind('article_show');
...
$app->post('/task/create', function (Request $request) use ($app) {
    ...
    return $app->redirect($app['url_generator']->generate('task_show', ['id' => $task->getId()]));
})->bind('task_create');
...
$app->get('/task/show/{id}', function ($id) use ($app) {
    ...
    return $app['twig']->render('task/show.html.twig', [
        'task' => $task,
    ]);
})->bind('task_show');
